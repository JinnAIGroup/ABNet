...
Run: on 3 terminals
   ...
   (YPN) jinn@Liu:~/YPN/ABNet$ python serverAB2B.py --port 5558 --validation
   (YPN) jinn@Liu:~/YPN/ABNet$ python train_modelB4.py --port 5557 --port_val 5558
Input:
...
Output:
  /ABNet/saved_model/modelB4_lossB.npy
...
from serverAB2B import client_generator, BATCH_SIZE
...
if __name__=="__main__":
    start = time.time()
    parser = argparse.ArgumentParser(description='Training modelB4')
    ...
    filepath = "./saved_model/modelB4-BestWeights.hdf5"
    checkpoint = ModelCheckpoint(filepath, monitor='val_loss', verbose=1,
                                 save_best_only=True, mode='min')
    callbacks_list = [checkpoint]

    #model.load_weights('./saved_model/modelB4-BestWeights.hdf5', by_name=True)
    ...
    np.save('./saved_model/modelB4_loss', np.array(history.history['loss']))
    lossnpy = np.load('./saved_model/modelB4_loss.npy')
