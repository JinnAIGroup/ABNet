...
2. B Tasks: temporal state (features) + path planning (PP)
...
Run: on 3 terminals
   (YPN) jinn@Liu:~/YPN/OPNet$ python serverAB2B.py --port 5557
   (YPN) jinn@Liu:~/YPN/OPNet$ python serverAB2B.py --port 5558 --validation
   (YPN) jinn@Liu:~/YPN/OPNet$ python train_modelAB2B.py --port 5557 --port_val 5558
Input:
...
Output:
  /OPNet/saved_model/modelAB_lossB.npy
...
from modelAB2B import get_model
...
if __name__=="__main__":
    ...
    np.save('./saved_model/modelAB_lossB', np.array(history.history['loss']))
    lossnpy = np.load('./saved_model/modelAB_lossB.npy')
