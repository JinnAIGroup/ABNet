...
Note: open only one ''' ... ''' when train A or B data, connot be both
...
Output:
  ...
  Ymasks = out12.shape = (None, 256, 512, 12)
"""
import os
import cv2
import h5py
...
def concatenate(img, msk, mask_H, mask_W, class_values):
    if os.path.isfile(img) and os.path.isfile(msk):
        ...
        print('#---datagenAB  Error: image_yuv.h5 or mask.png does not exist')

    return imgH5, mskCV2

def datagen(batch_size, camera_files, images, masks, image_H, image_W, mask_H, mask_W, class_values):
    ...
    for cfile, pfile, rfile in zip(camera_files, path_files, radar_files):
        if os.path.isfile(cfile) and os.path.isfile(pfile) and os.path.isfile(rfile):
            print('#---datagenAB  OK: cfile, pfile, rfile exist')
        else:
            print('#---datagenAB  Error: cfile, pfile, or rfile does not exist')
    Nplot = 0
